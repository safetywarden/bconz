...
export type PubTatorExtraction = { entities: PubTatorEntity[]; relations: PubTatorRelation[] };
...
function parseBioCJsonText(text: string): unknown[] {
  ...
  try {
    ...
    return asArray(record?.documents).length ? asArray(record?.documents) : record ? [record] : [];
  } catch {
    // PubTator BioCJSON may be returned as newline-delimited JSON documents.
...
export async function extractPubTator3(evidence: NormalizedEvidence[]): Promise<PubTatorExtraction> {
  ...
  const entities: PubTatorEntity[] = []; const relations: PubTatorRelation[] = [];
  ...
  for (let i = 0; i < pmids.length; i += 20) {
    ...
    if (!response.ok) continue; // enrichment failure must not destroy base ingestion
    const documents = parseBioCJsonText(await response.text());
  ...
  return { entities: [...new Map(entities.map((entity) => [entityKey(entity), entity])).values()], relations: [...new Map(relations.map((relation) => [relationKey(relation), relation])).values()] };
}
